    ...
    <Layout browserTitle='Showcase' description='Showcase'>
      ...
      <div className='grid grid-cols-1 mb-10 gap-7 xs:grid-cols-2 md:grid-cols-3'>
        {AnotherExample.map(a => (
          <InternalLink to={a.to} src={a.src} name={a.name} key={a.src} url={a.url} />
        ))}
      </div>
    </Layout>
  );
...
const InternalLink = ({
  ...
  return (
    <Link href={to}>
      <a className='flex-col overflow-hidden duration-300 ease-out rounded-lg shadow-lg hover:shadow-xl hover:scale-102'>
        <Image
          src={src}
